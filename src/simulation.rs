    ...
    fn check_contact(&self, o1: &Object, o2: &Object) -> Option<Contact> {
        match (o1.collider, o2.collider) {
            ...
            (Collider::Box(w1, h1, d1), Collider::Box(w2, h2, d2)) => {
                ...
                let (p1_sum, p2_sum, normal_sum, count) = points
                    .iter()
                    .map(|v| t1_inv * (t2 * v))
                    .filter(|p| {
                    ...
                    .chain(
                        edges
                            ...
                            .filter_map(|(p, v)| {
                                ...
                                let Some(i1) = intersections.next() else {
                                    return None;
                                };
                                let Some(i2) = intersections.next() else {
                                    return None;
                                };
                                // debug_assert_eq!(intersections.count(), 0);
                                let p1 = (i1 + i2) / 2.0;
                                let p2 = (p1 / p1.xyz().abs().max()) * 0.5;
                                let n = p1 - p2;
                                Some((
                                    t2 * p1,
                                    t2 * p2,
                    ...
                    .fold(
                        ...
                        |(p1_sum, p2_sum, n_sum, count), (p1, p2, normal)| {
                            (
                                p1_sum + p1,
                                p2_sum + p2,
                                n_sum + normal.normalize(),
                                count + 1,
                            )
                        },
                    );
                if count == 0 {
                    ...
                    Some(Contact {
                        points: (
                            Point3::from(p1_sum.xyz() / f64::from(count)),
                            Point3::from(p2_sum.xyz() / f64::from(count)),
                        ),
                        normal: normal_sum.xyz().normalize(),
                    })
                }
            }
        }
